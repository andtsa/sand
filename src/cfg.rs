...
use crate::lang::{Expr, Expression, Function, Program, Statement};
...
pub fn construct_cfg(ast: &Program) -> Result<Graph<CfgNode, (), Directed>> {
    ...
    for func in &ast.0 {
        ...
        build_cfg_func(&mut graph, func, entry, exit, &function_entries, &function_exits)?;
    }
...
fn build_cfg_expr(
    ...
    match &expr.expr {
        ...
        Expression::While { cond, body } => {
            let cond_node = graph.add_node(CfgNode::Expr(cond.as_ref().clone()));
            let body_entry = build_cfg_expr(graph, body, cond_node, function_entries, function_exits)?;
        ...
        Expression::Block { statements, expr: block_expr} => {
            ...
            if let Some(final_expr) = block_expr {
                current_node = build_cfg_expr(graph, final_expr, current_node, function_entries, function_exits)?;
            }
            ...
            for stmt in statements.iter().rev() {
                ...
                current_node = build_cfg_expr(graph, stmt_expr, current_node, function_entries, function_exits)?;
            }
        ...
        Expression::Call { fn_name, args} => {
            ...
            for arg in args {
                current_node = build_cfg_expr(graph, arg, current_node, function_entries, function_exits)?;
            }
