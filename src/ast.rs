...
fn build_program(pair: Pair<Rule>) -> Result<Program, AstError> {
    ...
    for child in pair.into_inner() {
        match child.as_rule() {
            Rule::function => {
                funcs.push(build_function(child)?);
            }
...
fn build_function(pair: Pair<Rule>) -> Result<Function, AstError> {
...
fn build_statement(pair: Pair<Rule>) -> Result<Statement, AstError> {
    ...
    match first.as_rule() {
        Rule::declaration => {
            ...
            let expr = build_expr(decl_inner.next().missing("declaration expression")?)?;
        ...
        Rule::assignment => {
            ...
            let name = a_inner
                ...
                .as_str()
                .to_string(); // identifier
            let expr = build_expr(a_inner.next().missing("assignment type")?)?;
...
fn build_expr(pair: Pair<Rule>) -> Result<Expression, AstError> {
    match pair.as_rule() {
        Rule::expression => {
        ...
        Rule::logic_or => build_logic_or(pair),
        Rule::logic_xor => build_logic_xor(pair),
        Rule::logic_and => build_logic_and(pair),
        Rule::equality => build_equality(pair),
        Rule::comparison => build_comparison(pair),
        Rule::add_sub => build_add_sub(pair),
        Rule::mul_div => build_mul_div(pair),
...
fn build_logic_or(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let inner = pair.into_inner();
    binop_fold(inner, build_logic_xor)
}

// logic_xor = { logic_and ~ (xor ~ logic_and)* }
fn build_logic_xor(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let inner = pair.into_inner();
    binop_fold(inner, build_logic_and)
}

// logic_and = { equality ~ (and ~ equality)* }
fn build_logic_and(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let inner = pair.into_inner();
    binop_fold(inner, build_equality)
}

// equality = { comparison ~ ( (eq | ne) ~ comparison )* }
fn build_equality(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let mut inner = pair.into_inner();
    let mut expr = build_comparison(inner.next().missing("eq expression")?)?;
    while let Some(op_pair) = inner.next() {
...
fn build_comparison(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let mut inner = pair.into_inner();
    let mut expr = build_add_sub(inner.next().missing("comp expression")?)?;
    while let Some(op_pair) = inner.next() {
        let rhs_pair = inner.next().missing("comp right")?;
        let rhs = build_add_sub(rhs_pair)?;
...
fn build_add_sub(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let mut inner = pair.into_inner();
    let mut expr = build_mul_div(inner.next().missing("add_sub expresison")?)?;
    while let Some(op_pair) = inner.next() {
        let rhs_pair = inner.next().missing("add_sub right")?;
        let rhs = build_mul_div(rhs_pair)?;
        let op = match op_pair.as_rule() {
            Rule::add => Bop::Plus,
...
fn build_mul_div(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let mut inner = pair.into_inner();
    let mut expr = build_power(inner.next().missing("mul_div expression")?)?;
    while let Some(op_pair) = inner.next() {
        let rhs_pair = inner.next().missing("mul_div right")?;
        let rhs = build_power(rhs_pair)?;
        let op = match op_pair.as_rule() {
...
fn build_power(pair: Pair<Rule>) -> Result<Expression, AstError> {
    let mut inner = pair.into_inner();
    let left_pair = inner.next().missing("power expression")?;
    let left = build_unary(left_pair)?;
...
fn build_unary(pair: Pair<Rule>) -> Result<Expression, AstError> {
...
fn build_primary(pair: Pair<Rule>) -> Result<Expression, AstError> {
    ...
    if s.starts_with('{') {
        ...
        for inner in pair.into_inner() {
            match inner.as_rule() {
                Rule::statement => {
                    let st = build_statement(inner)?;
                    statements.push(st);
                }
                Rule::expression => {
                    // trailing expression (the final expression inside the block)
                    expr = Some(Box::new(build_expr(inner)?));
                }
                other => {
                    return Err(AstError::UnexpectedRule {
        ...
        return Ok(Expression::Block { statements, expr });
    }
    ...
    match inner.as_rule() {
        ...
        Rule::identifier => Ok(Expression::Var(inner.as_str().to_string())),
        other => Err(AstError::UnexpectedRule {
...
fn build_if(pair: Pair<Rule>) -> Result<Expression, AstError> {
    ...
    let else_e = match else_pair {
        Some(p) => build_expr(p)?,
        None => Expression::Unit, // grammar allows missing else; use Unit as default
    };
...
fn build_call(pair: Pair<Rule>) -> Result<Expression, AstError> {
