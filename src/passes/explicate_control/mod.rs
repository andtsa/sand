...
use crate::lang::ops::{Bop, Uop};
use crate::compiler::structure::{FnName, Map, Range, VarName};
...
impl MirProgram {
    pub fn from_typed_program(prog: &th::TypedProgram) -> Self {
        let functions = prog
            .functions
            .iter()
            .map(|(name, func)| (name.clone(), lower_function(func)))
            .collect();
...
fn lower_function(func: &th::TypedFunction) -> MirFunction {
    let mut cx = FnCx::new(func.name.clone(), func.range, func.ret_type);

    let params = func
        .parameters
        .iter()
        .map(|p| {
            let local = cx.get_or_create_local(p.name.clone(), p.ty, p.range);
            MirParam {
                local,
                name: p.name.clone(),
                ty: p.ty,
    ...
    let entry = cx.lower_tail(&func.body);
...
impl FnCx {
    fn new(name: FnName, range: Range, ret_type: Ty) -> Self {
        Self {
            name,
            range,
            ret_type,
    ...
    fn new_block(&mut self, statements: Vec<Statement>, terminator: Terminator) -> BlockId {
    ...
    fn set_block(&mut self, id: BlockId, statements: Vec<Statement>, terminator: Terminator) {
    ...
    fn get_or_create_local(&mut self, name: VarName, ty: Ty, range: Range) -> LocalId {
    ...
    fn fresh_temp(&mut self, ty: Ty, range: Range) -> LocalId {
        let id = LocalId(self.locals.len());
        let name = VarName::generated(format!("__ec_tmp{}", self.next_temp));
        self.next_temp += 1;
    ...
    fn const_operand(expr: &th::Expr) -> Option<Operand> {
        match &expr.expr {
    ...
    fn var_operand(&self, name: &VarName) -> Operand {
        let local = *self
            .local_map
            .get(name)
    ...
    fn simple_operand(&self, expr: &th::Expr) -> Option<Operand> {
        if let Some(c) = Self::const_operand(expr) {
            return Some(c);
        }

        match &expr.expr {
            th::Expression::RVar(v) => Some(self.var_operand(v)),
            _ => None,
        }
    }

    fn assign_stmt(&self, dst: LocalId, value: RValue, range: Range) -> Statement {
        Statement::Assign {
            dst: Self::place(dst),
            value,
            range,
    ...
    fn goto_block(&mut self, target: BlockId) -> BlockId {
        self.new_block(Vec::new(), Terminator::Goto { target })
    }

    fn unit_assign_then_goto(&mut self, dst: LocalId, range: Range, target: BlockId) -> BlockId {
        self.new_block(
    ...
    fn lower_tail(&mut self, expr: &th::Expr) -> BlockId {
        match &expr.expr {
            ...
            _ => {
                let tmp = self.fresh_temp(expr.ty, expr.range);
    ...
    fn lower_statements(&mut self, statements: &[th::Statement], cont: BlockId) -> BlockId {
        statements
            .iter()
            .rev()
    ...
    fn lower_statement(&mut self, stmt: &th::Statement, cont: BlockId) -> BlockId {
        match stmt {
            th::Statement::Declaration {
                ...
                let dst = self.get_or_create_local(name.clone(), *ty, *range);
                self.lower_assign(val, dst, cont)
            }

            th::Statement::Assignment { name, range, val } => {
                let dst = self.get_or_create_local(name.clone(), val.ty, *range);
                self.lower_assign(val, dst, cont)
            }
    ...
    fn lower_assign(&mut self, expr: &th::Expr, dst: LocalId, cont: BlockId) -> BlockId {
        match &expr.expr {
            th::Expression::If { cond, t, f } => {
            ...
            th::Expression::UnOp { op, right } => {
                let r_tmp = self.fresh_temp(right.ty, right.range);
            ...
            th::Expression::BinOp { left, op, right } => {
                let l_tmp = self.fresh_temp(left.ty, left.range);
                let r_tmp = self.fresh_temp(right.ty, right.range);
            ...
            th::Expression::Call { fn_name, args } => {
                let arg_temps = args
                    .iter()
                    .map(|a| self.fresh_temp(a.ty, a.range))
                    .collect::<Vec<_>>();
    ...
    fn lower_effect(&mut self, expr: &th::Expr, cont: BlockId) -> BlockId {
        match &expr.expr {
            th::Expression::If { cond, t, f } => {
    ...
    fn lower_pred(&mut self, expr: &th::Expr, then_bb: BlockId, else_bb: BlockId) -> BlockId {
        match &expr.expr {
            ...
            th::Expression::BinOp {
                ...
                let cmp_tmp = self.fresh_temp(Ty::Bool, expr.range);
            ...
            th::Expression::RVar(v) => self.new_block(
                Vec::new(),
                Terminator::Branch {
