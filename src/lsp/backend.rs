...
impl Backend {
    pub async fn check_document(&self, uri: Url, text: String, mod_ref: ModuleRef) {
        let diagnostics = match ProgramModule::parse_module(&text, mod_ref) {
            Ok(modules) => {
                modules
                    .into_iter()
                    .flat_map(|program| {
                        ...
                        match program.uniquify() {
                            Ok(ast) => {
                                ...
                                match TypedProgram::from_ast_program(&ast) {
                                    ...
                                    Err(type_error) => {
                                        type_error_to_diagnostic(&uri, &text, type_error)
                                            .into_iter()
