//! Interpreter tests
//!
//! Each test compiles a program and runs it through **both** the typed-HIR and
//! MIR interpreters, asserting:
...
fn interpret_if_takes_true_branch() {
    assert_eq!(
        run_both("def main(): Int := if true then 1 else 2"),
        Expression::Int(1)
    );
}
...
fn interpret_if_takes_false_branch() {
    assert_eq!(
        run_both("def main(): Int := if false then 1 else 2"),
        Expression::Int(2)
    );
}
